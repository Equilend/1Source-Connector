...
public class TradeEventService implements EventService {
  ...
  public void processParties() {
    ...
    if (CollectionUtils.isEqualCollection(storedIds, retrievedIds)) {
      return;
    }
    ...
    for (TradeEvent event : events) {
      if (event.getEventType().equals(TRADE)) {
        ...
        if (agreementDto != null) {
          ...
          var recordRequest = eventBuilder.buildRequest(TRADE_AGREEMENT_CREATED, savedTradeEvent.getResourceUri());
