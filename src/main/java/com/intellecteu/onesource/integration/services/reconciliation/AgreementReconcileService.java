...
import static com.intellecteu.onesource.integration.model.enums.FieldSource.ONE_SOURCE_TRADE_AGREEMENT;
...
@Service
@Slf4j
public class AgreementReconcileService implements ReconcileService<Agreement, Position> {

    public static final String RECONCILE_EXCEPTION = """
        The trade agreement: %s is in discrepancies with the backoffice position id:%s""";

    public static final String RECONCILE_MISMATCH = "Reconciliation mismatch. The trade agreement %s:%s "
        + "is not matched with Backoffice position %s:%s";

    @Override
    public void reconcile(Agreement agreement, @NonNull Position backofficePosition) throws ReconcileException {
        var reconciliationExceptionDetails = new ArrayList<ProcessExceptionDetails>();
        var tradeAgreement = agreement.getTrade();
        reconciliationExceptionDetails.addAll(validateReconcilableObject(agreement, ONE_SOURCE_TRADE_AGREEMENT));
        ...
        if (!reconciliationExceptionDetails.isEmpty()) {
            ...
            throw new ReconcileException(
                format(RECONCILE_EXCEPTION, agreement.getAgreementId(), backofficePosition.getPositionId()),
    ...
    private List<ProcessExceptionDetails> reconcileRate(Rate rate, @NonNull Position position) {
        ...
        if (rate != null && rate.getRebate() != null && positionIndex != null) {
            ...
            if (floatingPositionRate(positionIndex)) {
                // todo: not in the scope of MVP. Requirements Should be updated
...
//                }
    ...
    Optional<ProcessExceptionDetails> checkEquality(@NonNull Object field, String fieldName,
        @NonNull Object positionField, String positionFieldName) {
        if (!Objects.equals(field, positionField)) {
            log.debug(format(RECONCILE_MISMATCH, field, fieldName, positionField, positionFieldName));
            return Optional.of(new ProcessExceptionDetails(
                ONE_SOURCE_TRADE_AGREEMENT,
                fieldName,
